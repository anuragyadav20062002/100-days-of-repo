...
{
    private:
    void inertattail(Node* &head,Node* &tail,int d){
         Node* nn=new Node(d);
        if(head==NULL){
           head = nn;
           tail =nn;
...
while(temp != NULL)
{
    inertattail(clonehead,clonetail,temp->data);
    temp = temp->next;
}
...
// clone node between original list
...
// connect random pointer in clone
temp = head;
while(temp != NULL)
{
    if(temp->next != NULL){
