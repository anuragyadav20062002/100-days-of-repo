...
class Solution {
public:
 Node* Solve(Node* root, unordered_map<int,Node*>& mp) {
        Node* newRoot = new Node();  //Creating a new node
        newRoot->val = root->val;
        vector<Node*> newVec = vector<Node*>();
        mp[root->val]=newRoot;  //Storing the visited new Node
        for(Node* v : root->neighbors) {
            if(mp.find(v->val) != mp.end())
              newVec.push_back(mp[v->val]);
            else
              newVec.push_back(Solve(v, mp));
        }
        newRoot->neighbors = newVec;
        return newRoot;
    }
